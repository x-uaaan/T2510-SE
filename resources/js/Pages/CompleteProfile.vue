<template>
  <BaseLayout>
    <div class="min-h-screen flex items-center justify-center bg-[#18191A]">
      ...
      <div v-if="showToast" class="toast">Register successfully</div>
    </div>
  </BaseLayout>
</template>
...
async function submit() {
  if (!validate()) {
    Object.keys(errors.value).forEach(key => {
      // Keep error toastify for validation errors
  ...
  try {
    await router.post('/complete-profile', formData, {
      onSuccess: () => {
        ...
        setTimeout(() => { showToast.value = false; router.visit('/events'); }, 2500);
      },
      onError: (errors) => {
