  ...
  <transition name="drawer-slide">
    <div v-if="event" class="drawer-backdrop" @click.self="close">
      <div class="drawer">
        ...
        <div v-if="showToast" class="toast">RSVP successfully</div>
        <div class="drawer-footer">
          <a :href="`mailto:${event.organiserEmail}`" class="drawer-link">Contact host</a>
...
const attendEvent = async () => {
  console.log('RSVP button clicked', { event: props.event, isAttending: isAttending.value });
  if (!props.event) { console.log('No event prop'); return; }
  if (!props.event.eventID) { console.log('No event id'); return; }
  if (isAttending.value) { console.log('Already attending'); return; }
  isAttending.value = true;
  try {
    await axios.get('/sanctum/csrf-cookie', { baseURL: '' });
    const response = await axios.post(
      '/events/' + props.event.eventID + '/attend',
      {
...
const checkIfAttending = async (eventId) => {
  ...
  try {
    ...
    await axios.get('/sanctum/csrf-cookie', { baseURL: '' });
    const response = await axios.get('/events/' + eventId + '/check-attendance', { baseURL: '' });
...
onMounted(async () => {
  try {
    ...
    console.log('Fetched userID on mount:', currentUserId.value);
  } catch (e) {
  ...
  if (props.event && props.event.eventID) {
    console.log('Drawer opened with eventID:', props.event.eventID);
    checkIfAttending(props.event.eventID);
  }
})
...
watch(() => props.event, (newEvent) => {
  if (newEvent && newEvent.eventID) {
    console.log('Drawer event changed, eventID:', newEvent.eventID);
    checkIfAttending(newEvent.eventID);
  } else {
    isAttending.value = false;
    isCheckingAttendance.value = false;
  }
