...
class AttendeeController extends Controller
{
    ...
    {
        try {
            $attendees = Attendee::where('eventID', $event->eventID)
                                ->with('user') // Load the user relationship
                                ->get();

            return response()->json($attendees);
        } catch (\Exception $e) {
            Log::error('Error fetching attendees: ' . $e->getMessage(), ['event_id' => $event->eventID]);
            return response()->json(['message' => 'Failed to fetch attendees.'], 500);
        }
    }
